...
The PCB the [rover power panel](../rover-power-panel) was missing. It sits on
...
**Status: designed, generated and checked. Nothing is ordered or built.**
Every number below is a calculation; [`measurements/`](measurements/) has a
template in it and nothing else. Seven datasheet facts were quoted from
memory rather than read off a PDF on disk, and the design checks print all
seven on every run — see [Before ordering](#before-ordering).

```
make            (in hw/)
== design self-checks ==      PASS   42 limits + 9 behaviour scenarios
== ERC ==                     0 errors, 0 warnings
== DRC (schematic parity) ==  0 violations, 0 unconnected pads
== interlock ==               PASS   24 structural checks, 6 of 6 mutations caught
== layout ==                  PASS   power path, Kelvin pair, part proximity
== fit ==                     PASS   board vs the panel's params.py, hole for hole
== all checks passed ==
```
...
The [plan](PLAN.md) is left as written. These are the places it was wrong or
incomplete:
...
```
rover-panel-supervisor/
├── PLAN.md                  the plan, as written before any of this existed
├── design/
│   ├── panel_interface.json GENERATED by ../rover-power-panel/scripts/params.py
│   ├── spec.py              every input, marked PANEL / DS / VERIFY / ASSUMED
│   ├── calcs.py             every derived number
│   └── test_calcs.py        limits + the interlock simulation
├── hw/
│   ├── Makefile             make / make check / make fab / make docs
│   ├── gen/
│   │   ├── gen_lib.py       the 74LVC1G74 symbol
│   │   ├── gen_sch.py       schematic from a pin -> net table
│   │   ├── gen_pro.py       project file and net classes
│   │   ├── gen_pcb.py       placement, routing, pours, silkscreen
│   │   ├── autoroute.py     the grid router
│   │   ├── kisch.py kipcb.py sexp.py symlib.py   reused from can-iso-breakout
│   │   ├── check_interlock.py  test_interlock_check.py
│   │   ├── check_layout.py  check_fit.py
│   │   └── debug_render.py  draws the router's view when a net will not route
│   ├── rover-panel-supervisor.kicad_sch / .kicad_pcb / .kicad_pro / .net   GENERATED
│   ├── fab/  rover-panel-supervisor-fab.zip   gerbers, drill, BOM, placement
│   └── lib/
├── docs/                    schematic PDF, board SVGs, 3D render
...
```bash
cd /Users/karimfatkhutdinov/uofm/Portfolio/rover-panel-supervisor/hw && make
```

That builds the symbol library, schematic, netlist, board and project, then
runs every check. `make fab` writes the fabrication package and `make docs`
the images. The Python checks use `python3`. Board generation and the three
board checks use KiCad 9's bundled interpreter, which the Makefile finds
inside `/Applications/KiCad`.

If the panel changes, re-export the interface first, then `make`. `check_fit`
fails until you do:

```bash
cd /Users/karimfatkhutdinov/uofm/Portfolio/rover-panel-supervisor/hw && make interface && make
```

To open it: `rover-panel-supervisor.kicad_pro` in KiCad 9. Edits belong in
`gen/`, not in the generated files.
